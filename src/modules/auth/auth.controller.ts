  ...
  async register(@Body() registerDto: RegisterDto, @Req() req: ExpRequest) : Promise<ResponseDto<any>> {
    try {
      ...
      throw new HttpException(
        new ResponseDto(false, `Failed to register user - ${errorMessage}`, null, errorMessage),
...
@Post('update-password-legacy')
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth('JWT-auth')
  @ApiOperation({ summary: 'Update user password' })
  @ApiResponse({ status: 200, description: 'Password updated successfully' })
  @ApiResponse({ status: 401, description: 'Unauthorized - invalid credentials' })
  updatePassword_legacy(@Request() req, @Body() updatePasswordDto: UpdatePasswordLegacyDto) {
    return this.authService.updatePassword_legacy(req.user.id, updatePasswordDto);
  }
