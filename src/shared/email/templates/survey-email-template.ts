// src/mail/templates/suhana-share.email.template.ts

export interface suhanaShareEmailParams {
  /** Full name of the suhana creator / organiser */
  organizerName:  string;
  /** Title of the suhana e.g. "Perambur Election suhana 2026" */
  suhanaTitle:    string;
  /** Media partner / channel name e.g. "ABC TV Channel" */
  mediaName?:     string;
  /** Full suhana vote URL (may contain spaces – will be encoded) */
  suhanaUrl:      string;
  /** Short alias used in the display URL e.g. "voter-pulse.com/s/abc123" */
  shortUrl:       string;
  /** suhana close date — JavaScript Date object */
  expiryDate:     Date;
...
export const suhanaShareEmailTemplate = (params: suhanaShareEmailParams): string => {
  const {
    organizerName,
    suhanaTitle,
    mediaName,
  ...
  const encodedUrl    = encodeUrl(suhanaUrl);
  ...
  const shareText = `🗳️ Cast your vote in "${suhanaTitle}"! Your voice matters. Vote before ${formattedDate}.`;

  // Social share URLs
  const waUrl  = whatsappShareUrl(`${shareText} ${suhanaUrl}`);
  const fbUrl  = facebookShareUrl(suhanaUrl);
  const twUrl  = twitterShareUrl(shareText, suhanaUrl);
  const liUrl  = linkedinShareUrl(suhanaUrl, suhanaTitle);
...
<head>
  ...
  <title>Vote Now – ${suhanaTitle}</title>
</head>
...
/**
 * Generates a deterministic short code from the suhana ID.
 * In production, call a proper URL shortener API here.
 */
export function buildShortUrl(suhanaId: string, baseDomain = 'voter-pulse.com'): string {
  // Take the first 10 alphanumeric chars of the suhana ID as a readable code
  const code = suhanaId.replace(/[^a-z0-9]/gi, '').slice(0, 10).toLowerCase();
  return `https://${baseDomain}/v/${code}`;
}


// ─────────────────────────────────────────────────────────────────────────────
//  Usage in your NestJS service (e.g. suhanas.service.ts)
// ─────────────────────────────────────────────────────────────────────────────
//
//  import { suhanaShareEmailTemplate, buildShortUrl } from '../mail/templates/suhana-share.email.template';
//
//  async publishsuhana(suhanaId: string, userId: string): Promise<void> {
//    const suhana = await this.suhanaRepo.findOne({ where: { id: suhanaId }, relations: [...] });
//
//    const suhanaUrl = `https://voter-pulse.com/${suhana.createdBy.username}/vote/${suhanaId}`;
//    const shortUrl  = buildShortUrl(suhanaId);  // or call Bitly / TinyURL API
//
//    const html = suhanaShareEmailTemplate({
//      organizerName: suhana.createdBy.fullName,
//      suhanaTitle:   suhana.name,
//      mediaName:     suhana.mediaPartner ?? undefined,
//      suhanaUrl,
//      shortUrl,
//      expiryDate:    new Date(suhana.endDate),
//      customMessage: suhana.shareMessage ?? undefined,
//      domain:        'voter-pulse.com',
//    });
//
//    await this.mailService.send({
//      to:      recipientEmail,        // or a list of recipients
//      subject: `🗳️ Vote now – ${suhana.name}`,
//      html,
//    });
//  }